...
const toPost = ({ post, category }: JoinedPost): Post => ({ title: post.title, slug: post.slug, date: (post.publishedAt ?? post.createdAt).toISOString().slice(0, 10), excerpt: post.excerpt, coverImage: post.coverImageUrl || '/home-maintenance-hero.png', coverAlt: post.coverAlt || post.title, category: category.slug, categoryName: category.name, tags: post.tags, author: post.authorId, content: post.content });
