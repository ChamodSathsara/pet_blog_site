...
export async function POST(request: Request) {
  try {
    ...
    const [post] = await db.insert(posts).values({ ...data, homepageFeatured, publishedAt: data.status === 'published' ? (data.publishedAt ? new Date(data.publishedAt) : new Date()) : null }).returning();
    const [category] = await db.select({ slug: categories.slug }).from(categories).where(eq(categories.id, data.categoryId)).limit(1);
    revalidateTag('posts'); revalidateTag('homepage-featured'); revalidateTag(`post-${post.slug}`);
