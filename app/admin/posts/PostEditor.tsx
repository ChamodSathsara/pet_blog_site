...
export function PostEditor({ initial, categories, authors }: { initial: Initial; categories: Option[]; authors: Option[] }) {
  ...
  const [uploadingCover, setUploadingCover] = useState(false); const [coverNotice, setCoverNotice] = useState('');
  ...
  const uploadInlineImage = async () => {
    ...
    let imageUrl: string;
    try { imageUrl = await uploadImage(inlineFile); }
    catch (uploadError) { setInlineError(uploadError instanceof Error ? uploadError.message : 'Could not upload image.'); return; }
    finally { setUploadingInline(false); }
    const safeAlt = alt.replace(/\]/g, '');
    const safeCaption = inlineCaption.trim().replace(/"/g, "'");
    const markdown = `![${safeAlt}](${imageUrl}${safeCaption ? ` "${safeCaption}"` : ''})`;
    const textarea = editorRef.current?.querySelector('textarea');
    const start = textarea?.selectionStart ?? form.content.length;
    const end = textarea?.selectionEnd ?? start;
    const before = form.content.slice(0, start);
  ...
  return <form onSubmit={submit} className="mt-6 space-y-5">
    ...
    <div className="grid gap-5 md:grid-cols-2"><Field label="Cover image URL"><Input value={form.coverImageUrl} onChange={(e) => set('coverImageUrl', e.target.value)} /><input className="mt-2 text-sm" type="file" disabled={uploadingCover} accept="image/jpeg,image/png,image/webp,image/gif" onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; setUploadingCover(true); setError(''); setCoverNotice(''); try { const url = await uploadImage(file); set('coverImageUrl', url); setCoverNotice('Cover image uploaded.'); } catch (uploadError) { setError(uploadError instanceof Error ? uploadError.message : 'Could not upload cover image.'); } finally { setUploadingCover(false); } }} />{uploadingCover && <span className="mt-2 block text-sm text-muted-foreground">Uploading cover image…</span>}{coverNotice && <span className="mt-2 block text-sm font-medium text-primary">{coverNotice}</span>}</Field><Field label="Cover alt text"><Input required={Boolean(form.coverImageUrl)} placeholder="Describe what is visible in the image" value={form.coverAlt} onChange={(e) => set('coverAlt', e.target.value)} /><span className="mt-1 block text-xs text-muted-foreground">Required when a cover image is used. Describe the image naturally; do not stuff keywords.</span></Field></div>
    ...
    {error && <p role="alert" className="text-destructive">{error}</p>}<div className="flex gap-3"><Button size="lg" disabled={saving || uploadingCover || uploadingInline}>{saving ? 'Saving…' : uploadingCover || uploadingInline ? 'Uploading image…' : 'Save post'}</Button><Button type="button" size="lg" variant="outline" onClick={() => router.push('/admin/posts')}>Cancel</Button></div>
  </form>;
}
