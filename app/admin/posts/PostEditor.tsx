...
type Initial = { id?: string; title: string; slug: string; excerpt: string; seoTitle: string; metaDescription: string; canonicalUrl: string; noIndex: boolean; content: string; coverImageUrl: string; coverAlt: string; categoryId: string; authorId: string | null; tags: string[]; status: 'draft' | 'published' };
...
export function PostEditor({ initial, categories, authors }: { initial: Initial; categories: Option[]; authors: Option[] }) {
  ...
  return <form onSubmit={submit} className="mt-6 space-y-5">
    ...
    <Field label="Content (Markdown)"><div ref={editorRef} data-color-mode="light"><MDEditor value={form.content} onChange={(value) => set('content', value || '')} height={520} commandsFilter={(command) => command.name === 'image' ? { ...command, execute: () => { document.getElementById('inline-image-upload')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); window.setTimeout(() => inlineFileRef.current?.focus(), 350); } } : command} components={{ preview: (source: string) => <div className="px-5 pb-8"><Markdown source={source} /></div> }} /></div></Field>
